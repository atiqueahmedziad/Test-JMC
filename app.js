...
const cat = dataSlice.forEach(data => {
...
const handleChange = (checkbox) => {
  ...
  if(!productListLength) {
    ...
    //console.log("content after checked in ",content);
    ...
    const filterout = filterContent.map(eachData => {
...
const allcategories = dataSlice.forEach(data => {
...
let countsObject = {};
allCategories.forEach(x => countsObject[x] = (countsObject[x] || 0)+1 );
	...
	return `
		<ul class="filter-options">	
			${categories
				.map((category, i) => {
          return `
          <li><input type="checkbox" onchange="handleChange(this)" name="${category}" 
					value="${category}">${category} <span class="product-present">${countValue[i]}</span</li>
          `})
				.join('')
			}
		</ul>`
